...
- Full PDF manual: docs/LaTeX/budget_analyser_documentation.tex (compiled PDF at docs/budget_analyser_documentation.pdf)
- Architecture: docs/HLD.md (high-level) and docs/LLD.md (low-level)
- Migration plan: docs/VERTICAL_SLICES_MIGRATION.md (vertical slices roadmap and status)
- UML diagrams: docs/uml/*.puml (exported PNGs are generated by the update script)
...
- Packages under src/budget_analyser/
  - core: shared foundation (protocols, errors, database utilities, cross-feature DTOs)
  - features: vertical feature slices — self-contained modules owning all layers
    - budget_goals: COMPLETE pilot (models, repository, service, controller, page)
    - (net_worth, recurring, savings, etc. — planned, see VERTICAL_SLICES_MIGRATION.md)
  - views: Qt widgets and windows (LoginWindow, DashboardWindow, pages, shared widgets)
  - controller: legacy controllers being migrated to features/ (BudgetController is now a facade)
  - domain: legacy business logic being migrated to features/ (protocols/errors now in core/)
