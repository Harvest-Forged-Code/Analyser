...
@pytest.fixture()
def repo(tmp_path: Path) -> BudgetGoalsModel:
    """Create a model backed by a temporary database."""
    db_path = tmp_path / "test_budget.db"
    return BudgetGoalsModel(db_path=db_path)


# ==================== Budget Goals ====================


def test_set_and_get_budget_goal(repo: BudgetGoalsModel) -> None:
...
def test_set_budget_goal_upserts(repo: BudgetGoalsModel) -> None:
...
def test_get_budget_goal_month_specific(repo: BudgetGoalsModel) -> None:
...
def test_get_budget_goal_not_found(repo: BudgetGoalsModel) -> None:
    result = repo.get_budget_goal("NonExistent")
    assert result is None


def test_get_all_budget_goals(repo: BudgetGoalsModel) -> None:
    repo.set_budget_goal("Food", 500.0)
...
def test_delete_budget_goal(repo: BudgetGoalsModel) -> None:
    repo.set_budget_goal("Food", 500.0)
    assert repo.delete_budget_goal("Food") is True
    assert repo.get_budget_goal("Food") is None


def test_delete_budget_goal_not_found(repo: BudgetGoalsModel) -> None:
    assert repo.delete_budget_goal("NonExistent") is False


def test_set_budget_goals_for_year(repo: BudgetGoalsModel) -> None:
...
def test_set_and_get_earnings_goal(repo: BudgetGoalsModel) -> None:
...
def test_set_earnings_goal_upserts(repo: BudgetGoalsModel) -> None:
...
def test_get_earnings_goal_not_found(repo: BudgetGoalsModel) -> None:
    assert repo.get_earnings_goal("nonexistent") is None


def test_get_all_earnings_goals(repo: BudgetGoalsModel) -> None:
    repo.set_earnings_goal("salary", 5000.0)
...
def test_delete_earnings_goal(repo: BudgetGoalsModel) -> None:
    repo.set_earnings_goal("salary", 5000.0)
    assert repo.delete_earnings_goal("salary") is True
    assert repo.get_earnings_goal("salary") is None


def test_delete_earnings_goal_not_found(repo: BudgetGoalsModel) -> None:
    assert repo.delete_earnings_goal("nonexistent") is False


def test_set_earnings_goals_for_year(repo: BudgetGoalsModel) -> None:
