"""Yearly summary statistics service.
...
class YearlySummaryService:
    """Service to compute Home page statistics from MonthlyReports.

    Pure Python (no Qt). Returns DTOs for the view to render.

    Example:
        >>> svc = YearlySummaryService(reports, logger)
        >>> years = svc.available_years()
        >>> stats = svc.get_yearly_stats(2025)
    """
    ...
    def get_category_breakdown(  # pylint: disable=too-many-locals
        self,
        year: int,
    ) -> YearlyCategoryBreakdown:
        ...
        Example:
            >>> breakdown = svc.get_category_breakdown(2025)
...
YearlySummaryStatsController = YearlySummaryService
