...
class MappingValidationService:
        ...
        for keyword, sub_categories in keyword_to_subs.items():
            if len(sub_categories) > 1:
                report.add(ValidationIssue(
                    ...
                    message=(
                        f"Keyword '{keyword}' maps to multiple"
                        " sub-categories"
                    ),
                    details={
                        "keyword": keyword,
    ...
    def _check_orphan_sub_categories(
        ...
        for orphan in orphans:
            report.add(ValidationIssue(
                ...
                message=(
                    f"Sub-category '{orphan}' has no parent category"
                ),
    ...
    def _check_name_for_typos(
        ...
        for char in self.SUSPICIOUS_CHARS:
            if char in name:
                report.add(ValidationIssue(
                    ...
                    message=(
                        f"{name_type.title()} '{name}' contains"
        ...
        if "_" in name and any(
            c.isupper() for c in name.replace("_", "")
        ):
            words = name.split("_")
            styles = set()
            for word in words:
        ...
        for sub_category, keywords in desc_to_sub.items():
            if not keywords:
                report.add(ValidationIssue(
                    ...
                    message=(
                        f"Sub-category '{sub_category}' has no keywords"
                    ),
        ...
        for category, sub_categories in sub_to_cat.items():
            if not sub_categories:
                report.add(ValidationIssue(
                    ...
                    issue_type="empty_mapping",
                    message=(
                        f"Category '{category}' has no sub-categories"
                    ),
    ...
    def _check_similar_category_names(
        ...
        """Detect category names that are suspiciously similar."""
    ...
    def _check_cashflow_coverage(
        ...
        """Check that all categories are covered by cashflow."""
        ...
        for category in unclassified:
            report.add(ValidationIssue(
                ...
                message=(
                    f"Category '{category}' not in"
                    " earnings/expenses cashflow"
                ),
                details={"category": category},
            ))
