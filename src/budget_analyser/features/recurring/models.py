"""Recurring transactions feature models and persistence.

Data transfer objects and SQLite-backed storage for recurring
...
class RecurringModel:
    ...
    Example:
        ...
        >>> txn = model.add_recurring_transaction(
    ...
    def __init__(
        ...
        """Initialize the recurring transactions model.
    ...
    def add_recurring_transaction(  # pylint: disable=too-many-positional-arguments
        ...
        if row is None:
            raise RuntimeError(
                f"Failed to insert recurring transaction: "
                f"{description}"
            )
    ...
    def update_last_occurrence(
        ...
        Example:
            >>> model.update_last_occurrence(1, "2024-02-15")
            True
        """
        with get_connection(self._db_path) as conn:
            cursor = conn.execute(f"""
    ...
    def deactivate_recurring_transaction(
        ...
        Example:
            >>> model.deactivate_recurring_transaction(1)
            True
        """
        with get_connection(self._db_path) as conn:
            cursor = conn.execute(f"""
    ...
    def delete_recurring_transaction(
        ...
        Example:
            >>> model.delete_recurring_transaction(1)
            True
        """
        with get_connection(self._db_path) as conn:
            cursor = conn.execute(f"""
    ...
    def detect_recurring_transactions(
        ...
        Example:
            >>> detected = model.detect_recurring_transactions(
            ...     transactions_df=df,
            ...     min_occurrences=3,
            ... )
    ...
    def _estimate_frequency(
        ...
        Example:
            >>> RecurringModel._estimate_frequency(
...
RecurringRepository = RecurringModel
